...
class SubscriberNode : public rclcpp::Node {
 public:
     ...
     * Initializes the subscriber to subscribe to the "topic" and sets up a timer to monitor activity.
     */
    SubscriberNode() : Node("subscriber_node"), last_message_time_(now()) {
        ...
        inactivity_timer_ = this->create_wall_timer(1000ms, std::bind(&SubscriberNode::check_inactivity, this));
 ...
 private:
    ...
    /**
     * @brief Checks if no messages have been received for a period of time and logs a warning if idle.
     */
    void check_inactivity() {
        auto current_time = now();
        auto time_since_last_msg = std::chrono::duration_cast<std::chrono::seconds>(current_time - last_message_time_).count();
        
        if (time_since_last_msg > 2) {  // If more than 2 seconds have passed since the last message
            RCLCPP_WARN_STREAM(this->get_logger(), "No messages received in the last " << time_since_last_msg << " seconds.");
        }
    }
    ...
    rclcpp::Subscription<std_msgs::msg::String>::SharedPtr subscriber_; /**< Subscriber to receive messages from the topic. */
    rclcpp::TimerBase::SharedPtr inactivity_timer_; /**< Timer to monitor inactivity on the topic. */
    std::chrono::steady_clock::time_point last_message_time_; /**< Time point of the last received message. */
};
